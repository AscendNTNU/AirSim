...
void ASimModeWorldBase::initializeForPlay() {
    ...
    for (auto& api : getApiProvider()->getVehicleSimApis()) vehicles.push_back(api);
    ...
    physics_world_.reset(new msr::airlib::PhysicsWorld(std::move(physics_engine), vehicles, getPhysicsLoopPeriod()));
}
...
std::unique_ptr<ASimModeWorldBase::PhysicsEngineBase> ASimModeWorldBase::createPhysicsEngine() {
    ...
    if (physics_engine_name == "")
        ...
        physics_engine.reset();
        UAirBlueprintLib::LogMessageString("Unrecognized physics engine name: ", physics_engine_name,
...
void ASimModeWorldBase::Tick(float DeltaSeconds) {
    ...
    for (auto& api : getApiProvider()->getVehicleSimApis()) api->updateRendering(DeltaSeconds);

    Super::Tick(DeltaSeconds);
}

void ASimModeWorldBase::reset() {
    UAirBlueprintLib::RunCommandOnGameThread([this]() { physics_world_->reset(); }, true);
