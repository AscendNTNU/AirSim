...
private:
    void setupFrameGenericQuad(Params& params)
    {
		...
		*/
		...
		// Set up arm lengths, dimensions are for Ascend's XWing
		params.rotor_count = 4;
		std::vector<real_T> arm_lengths(params.rotor_count, 0.5f);

		// Set up mass
		real_T motor_assembly_weight = 0.213f;  //weight for XING 400 KV
		params.mass = 10.0f - params.rotor_count * motor_assembly_weight;
		real_T box_mass = params.mass - params.rotor_count * motor_assembly_weight;
		
		// Set up parameters for caluclating max thrust
		params.rotor_params.max_rpm = 400.0f * 8.0f * 4.2f; // 400 KV * cells in series * voltage per cell
		params.rotor_params.propeller_diameter = 18.0f / 39.27f; // 18 inch converted to meters

		// This is just an estimate
		real_T max_thrust = 9.80665f * 7.0f; // Per propellar
		real_T max_torque = 21.7f;
		params.rotor_params.C_T = max_thrust / (params.rotor_params.air_density * pow(params.rotor_params.max_rpm / 60.0f, 2) * pow(params.rotor_params.propeller_diameter, 4));
		params.rotor_params.C_P = max_torque / (params.rotor_params.air_density * pow(params.rotor_params.max_rpm / 60.0f, 2) * pow(params.rotor_params.propeller_diameter, 5));

		// This is a hack in order to make the drone be able to fly at the speed our real drone flies and not oscillate because of the 
		// motors saturating. 
		params.linear_drag_coefficient = 0;
        ...
        //set up dimensions of core body box or abdomen (not including arms).
        params.body_box.x() = 0.335f; params.body_box.y() = 0.23f; params.body_box.z() = 0.04f;
