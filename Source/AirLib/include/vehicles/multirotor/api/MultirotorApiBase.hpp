...
protected:
    float back_propeller_control_signal = 0.0;

public:
    float getBackPropellerControlSignal() const;
...
protected:    // types
    ...
    public:
        SingleTaskCall(MultirotorApiBase* api) : SingleCall(api) {
            if (isRootCall()) api->beforeTask();
        }

        virtual ~SingleTaskCall() {
            if (isRootCall()) getVehicleApi()->afterTask();
        }
    };
