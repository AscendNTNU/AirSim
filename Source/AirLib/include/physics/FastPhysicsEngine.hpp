...
public:
    FastPhysicsEngine(bool enable_ground_lock = true) : enable_ground_lock_(enable_ground_lock) {}
...
private:
    ...
    static Wrench getDragWrench(const PhysicsBody& body, const Quaternionr& orientation, const Vector3r& linear_vel,
        ...
        for (uint vi = 0; vi < body.dragVertexCount(); ++vi) {
            ...
            const Vector3r vel_vertex = VectorMath::transformToBodyFrame(linear_vel, orientation) +
                                        angular_vel_body.cross(vertex.getPosition());
