...
public:
    ...
    virtual void setBackPropellerControlSignal(const float& value, const std::string& vehicle_name = "") {
        unused(value);
    }
    ...
    */
    ...
    virtual ImuBase::Output getImuData(const std::string& imu_name) const {
        ...
        for (uint i = 0; i < count_imus; i++) {
            const ImuBase* current_imu =
                static_cast<const ImuBase*>(getSensors().getByType(SensorBase::SensorType::Imu, i));
    ...
    virtual BarometerBase::Output getBarometerData(const std::string& barometer_name) const {
        ...
        for (uint i = 0; i < count_barometers; i++) {
            const BarometerBase* current_barometer =
                static_cast<const BarometerBase*>(getSensors().getByType(SensorBase::SensorType::Barometer, i));
            if (current_barometer != nullptr &&
                (current_barometer->getName() == barometer_name || barometer_name == "")) {
    ...
    virtual MagnetometerBase::Output getMagnetometerData(const std::string& magnetometer_name) const {
        ...
        for (uint i = 0; i < count_magnetometers; i++) {
            const MagnetometerBase* current_magnetometer =
                static_cast<const MagnetometerBase*>(getSensors().getByType(SensorBase::SensorType::Magnetometer, i));
            if (current_magnetometer != nullptr &&
                (current_magnetometer->getName() == magnetometer_name || magnetometer_name == "")) {
    ...
    virtual GpsBase::Output getGpsData(const std::string& gps_name) const {
        ...
        for (uint i = 0; i < count_gps; i++) {
            const GpsBase* current_gps =
                static_cast<const GpsBase*>(getSensors().getByType(SensorBase::SensorType::Gps, i));
    ...
    virtual float getBackPropellerControlSignal() const { return 0.0; }
...
private:
    const LidarBase* findLidarByName(const std::string& lidar_name) const {
        ...
        for (uint i = 0; i < count_lidars; i++) {
            const LidarBase* current_lidar =
                static_cast<const LidarBase*>(getSensors().getByType(SensorBase::SensorType::Lidar, i));
