...
RpcLibServerBase::RpcLibServerBase(ApiProvider* api_provider, const std::string& server_address, uint16_t port)
    ...
    pimpl_->server.bind(
        "simGetImages",
        [&](const std::vector<RpcLibAdapatorsBase::ImageRequest>& request_adapter,
            const std::string& vehicle_name) -> vector<RpcLibAdapatorsBase::ImageResponse> {
            const auto& response =
                getVehicleSimApi(vehicle_name)->getImages(RpcLibAdapatorsBase::ImageRequest::to(request_adapter));
    ...
    pimpl_->server.bind("simGetImage",
                        [&](const std::string& camera_name, ImageCaptureBase::ImageType type,
                            ...
                            if (result.size() == 0) {
                                // rpclib has a bug with serializing empty vectors, so we return a 1 byte vector
                                // instead.
                                result.push_back(0);
                            }
                            return result;
    ...
    pimpl_->server.bind(
        "simSetVehiclePose",
        [&](const RpcLibAdapatorsBase::Pose& pose, bool ignore_collision, const std::string& vehicle_name) -> void {
    ...
    pimpl_->server.bind("simGetLidarSegmentation",
                        [&](const std::string& lidar_name, const std::string& vehicle_name) -> std::vector<int> {
    ...
    pimpl_->server.bind(
        "getLidarData",
        [&](const std::string& lidar_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::LidarData {
    ...
    pimpl_->server.bind(
        "getImuData",
        [&](const std::string& imu_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::ImuData {
    ...
    pimpl_->server.bind(
        "getBarometerData",
        [&](const std::string& barometer_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::BarometerData {
    ...
    pimpl_->server.bind("getMagnetometerData",
                        [&](const std::string& magnetometer_name,
                            const std::string& vehicle_name) -> RpcLibAdapatorsBase::MagnetometerData {
    ...
    pimpl_->server.bind(
        "getGpsData",
        [&](const std::string& gps_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::GpsData {
    ...
    pimpl_->server.bind("getDistanceSensorData",
                        [&](const std::string& distance_sensor_name,
                            const std::string& vehicle_name) -> RpcLibAdapatorsBase::DistanceSensorData {
    ...
    pimpl_->server.bind(
        "simGetCameraInfo",
        [&](const std::string& camera_name, const std::string& vehicle_name) -> RpcLibAdapatorsBase::CameraInfo {
    ...
    pimpl_->server.bind("simSetCameraOrientation",
                        [&](const std::string& camera_name, const RpcLibAdapatorsBase::Quaternionr& orientation,
    ...
    pimpl_->server.bind("simGetCollisionInfo",
                        [&](const std::string& vehicle_name) -> RpcLibAdapatorsBase::CollisionInfo {
    ...
    pimpl_->server.bind(
        "simSetObjectPose",
        [&](const std::string& object_name, const RpcLibAdapatorsBase::Pose& pose, bool teleport) -> bool {
            return getWorldSimApi()->setObjectPose(object_name, pose.to(), teleport);
        });

    pimpl_->server.bind(
        "simGetGroundTruthKinematics", [&](const std::string& vehicle_name) -> RpcLibAdapatorsBase::KinematicsState {
    ...
    pimpl_->server.bind("simGetGroundTruthEnvironment",
                        [&](const std::string& vehicle_name) -> RpcLibAdapatorsBase::EnvironmentState {
    ...
    pimpl_->server.bind(
        "simCharSetFaceExpression",
        [&](const std::string& expression_name, float value, const std::string& character_name) -> void {
            getWorldSimApi()->charSetFaceExpression(expression_name, value, character_name);
        });
    pimpl_->server.bind("simCharGetFaceExpression",
                        [&](const std::string& expression_name, const std::string& character_name) -> float {
    ...
    pimpl_->server.bind("simCharSetHeadRotation",
                        [&](const RpcLibAdapatorsBase::Quaternionr& q, const std::string& character_name) -> void {
                            getWorldSimApi()->charSetHeadRotation(q.to(), character_name);
                        });
    pimpl_->server.bind("simCharGetHeadRotation",
                        [&](const std::string& character_name) -> RpcLibAdapatorsBase::Quaternionr {
                            msr::airlib::Quaternionr q = getWorldSimApi()->charGetHeadRotation(character_name);
                            return RpcLibAdapatorsBase::Quaternionr(q);
                        });
    pimpl_->server.bind("simCharSetBonePose",
                        [&](const std::string& bone_name, const RpcLibAdapatorsBase::Pose& pose,
                            const std::string& character_name) -> void {
                            getWorldSimApi()->charSetBonePose(bone_name, pose.to(), character_name);
                        });
    pimpl_->server.bind(
        "simCharGetBonePose",
        [&](const std::string& bone_name, const std::string& character_name) -> RpcLibAdapatorsBase::Pose {
            msr::airlib::Pose pose = getWorldSimApi()->charGetBonePose(bone_name, character_name);
            return RpcLibAdapatorsBase::Pose(pose);
        });
    pimpl_->server.bind("simCharResetBonePose",
                        [&](const std::string& bone_name, const std::string& character_name) -> void {
                            getWorldSimApi()->charResetBonePose(bone_name, character_name);
                        });
    pimpl_->server.bind("simCharSetFacePreset",
                        [&](const std::string& preset_name, float value, const std::string& character_name) -> void {
                            getWorldSimApi()->charSetFacePreset(preset_name, value, character_name);
                        });
    pimpl_->server.bind(
        "simSetFacePresets",
        [&](const std::unordered_map<std::string, float>& presets, const std::string& character_name) -> void {
            getWorldSimApi()->charSetFacePresets(presets, character_name);
        });
    pimpl_->server.bind("simSetBonePoses",
                        [&](const std::unordered_map<std::string, RpcLibAdapatorsBase::Pose>& poses,
                            const std::string& character_name) -> void {
                            std::unordered_map<std::string, msr::airlib::Pose> r;
                            for (const auto& p : poses) r[p.first] = p.second.to();
    ...
    pimpl_->server.bind(
        ...
        [&](const std::vector<std::string>& bone_names,
            const std::string& character_name) -> std::unordered_map<std::string, RpcLibAdapatorsBase::Pose> {
            std::unordered_map<std::string, msr::airlib::Pose> poses =
                getWorldSimApi()->charGetBonePoses(bone_names, character_name);
            std::unordered_map<std::string, RpcLibAdapatorsBase::Pose> r;
            for (const auto& p : poses) r[p.first] = RpcLibAdapatorsBase::Pose(p.second);
