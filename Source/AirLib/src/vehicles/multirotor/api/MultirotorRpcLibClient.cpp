...
__pragma(warning(disable : 4239))
    ...
    MultirotorRpcLibClient* MultirotorRpcLibClient::moveByAngleZAsync(float pitch, float roll, float z, float yaw,
                                                                      float duration, const std::string& vehicle_name) {
        pimpl_->last_future = static_cast<rpc::client*>(getClient())
                                  ->async_call("moveByAngleZ", pitch, roll, z, yaw, duration, vehicle_name);
        return this;
    }

    MultirotorRpcLibClient* MultirotorRpcLibClient::moveByAngleThrottleAsync(float pitch, float roll, float throttle,
                                                                             float yaw_rate, float duration,
                                                                             const std::string& vehicle_name) {
        pimpl_->last_future =
            static_cast<rpc::client*>(getClient())
                ->async_call("moveByAngleThrottle", pitch, roll, throttle, yaw_rate, duration, vehicle_name);
        return this;
    }
    ...
    MultirotorRpcLibClient* MultirotorRpcLibClient::moveByVelocityAsync(float vx, float vy, float vz, float duration,
        ...
        pimpl_->last_future = static_cast<rpc::client*>(getClient())
                                  ->async_call("moveByVelocity", vx, vy, vz, duration, drivetrain,
                                               MultirotorRpcLibAdapators::YawMode(yaw_mode), vehicle_name);
        return this;
    }
    ...
    MultirotorRpcLibClient* MultirotorRpcLibClient::moveToZAsync(float z, float velocity, float timeout_sec,
        ...
        pimpl_->last_future =
            static_cast<rpc::client*>(getClient())
                ->async_call("moveToZ", z, velocity, timeout_sec, MultirotorRpcLibAdapators::YawMode(yaw_mode),
    ...
    bool MultirotorRpcLibClient::setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance,
        ...
        return static_cast<rpc::client*>(getClient())
            ->call("setSafety", static_cast<uint>(enable_reasons), obs_clearance, obs_startegy, obs_avoidance_vel,
                   MultirotorRpcLibAdapators::Vector3r(origin), xy_length, max_z, min_z, vehicle_name)
            .as<bool>();
    }
    ...
    MultirotorState MultirotorRpcLibClient::getMultirotorState(const std::string& vehicle_name) {
        return static_cast<rpc::client*>(getClient())
            ...
            .as<MultirotorRpcLibAdapators::MultirotorState>()
            .to();
    }

    void MultirotorRpcLibClient::moveByRC(const RCData& rc_data, const std::string& vehicle_name) {
        static_cast<rpc::client*>(getClient())
            ->call("moveByRC", MultirotorRpcLibAdapators::RCData(rc_data), vehicle_name);
    }

    MultirotorRpcLibClient* MultirotorRpcLibClient::setBackPropellerControlSignal(const float& value,
                                                                                  const std::string& vehicle_name) {
        pimpl_->last_future =
            static_cast<rpc::client*>(getClient())->async_call("setBackPropellerControlSignal", value, vehicle_name);
        return this;
    }
    ...
    MultirotorRpcLibClient* MultirotorRpcLibClient::waitOnLastTask(bool* task_result, float timeout_sec) {
        ...
        if (task_result) *task_result = result;
