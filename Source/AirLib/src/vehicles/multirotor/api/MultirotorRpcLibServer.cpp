...
MultirotorRpcLibServer::MultirotorRpcLibServer(ApiProvider* api_provider, string server_address, uint16_t port)
    ...
    (static_cast<rpc::server*>(getServer()))
        ->bind(
            "moveByAngleZ",
            [&](float pitch, float roll, float z, float yaw, float duration, const std::string& vehicle_name) -> bool {
                return getVehicleApi(vehicle_name)->moveByAngleZ(pitch, roll, z, yaw, duration);
            });
    (static_cast<rpc::server*>(getServer()))
        ->bind("moveByAngleThrottle",
               [&](float pitch, float roll, float throttle, float yaw_rate, float duration,
    ...
    (static_cast<rpc::server*>(getServer()))
        ->bind("moveOnPath",
               [&](const vector<MultirotorRpcLibAdapators::Vector3r>& path, float velocity, float timeout_sec,
                   DrivetrainType drivetrain, const MultirotorRpcLibAdapators::YawMode& yaw_mode, float lookahead,
    ...
    (static_cast<rpc::server*>(getServer()))
        ->bind("moveToZ",
               [&](float z, float velocity, float timeout_sec, const MultirotorRpcLibAdapators::YawMode& yaw_mode,
    ...
    (static_cast<rpc::server*>(getServer()))
        ->bind("setBackPropellerControlSignal", [&](const float& value, const std::string& vehicle_name) -> void {
            getVehicleApi(vehicle_name)->setBackPropellerControlSignal(value);
        });
    ...
    // getters
    (static_cast<rpc::server*>(getServer()))
        ->bind("getMultirotorState",
               [&](const std::string& vehicle_name) -> MultirotorRpcLibAdapators::MultirotorState {
