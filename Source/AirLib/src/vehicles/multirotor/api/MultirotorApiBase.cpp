    ...
    while (!waiter.isTimeout() && (next_path_loc.seg_index < path_segs.size() - 1 ||
        ...
        */
                ...
                if (error > lookahead_error) {
                    ...
                    if (lookahead_error_increasing > 1E5) {
                        throw std::runtime_error(
                            "lookahead error is continually increasing so we do not have safe control, aborting "
                            "moveOnPath operation");
                    }
                } else {
        ...
        if (goal_dist >= 0) {
            ...
            if (overshoot) Utils::log(Utils::stringf("overshoot=%f", overshoot));
        }
        // else
...
bool MultirotorApiBase::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain,
    ...
    do {
        ...
        if (rc_data.is_valid && (rc_data.timestamp == 0 ||
                                 age <= kMaxMessageAge)) {    // if rc message timestamp is not set OR is not too old
            if (rc_data_trims_.is_valid) rc_data.subtract(rc_data_trims_);
...
float MultirotorApiBase::getBackPropellerControlSignal() const { return back_propeller_control_signal; }

void MultirotorApiBase::setBackPropellerControlSignal(const float& value, const std::string& vehicle_name) {
    back_propeller_control_signal = value;
}
...
bool MultirotorApiBase::rotateToYaw(float yaw, float timeout_sec, float margin) {
    ...
    do {
        auto kinematics = getKinematicsEstimated();
        VectorMath::toEulerianAngle(kinematics.pose.orientation, estimated_pitch, estimated_roll, estimated_yaw);

        if (isYawWithinMargin(estimated_yaw, margin)) return true;
...
Waiter MultirotorApiBase::waitForFunction(WaitFunction function, float timeout_sec) {
    Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
    if (timeout_sec <= 0) return waiter;
...
void MultirotorApiBase::moveToPathPosition(const Vector3r& dest, float velocity, DrivetrainType drivetrain,
    ...
    unused(last_z);
    // validate dest
    if (dest.hasNaN()) throw std::invalid_argument(VectorMath::toString(dest, "dest vector cannot have NaN: "));
