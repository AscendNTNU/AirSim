...
void MultirotorPawnSimApi::initialize() {
    ...
    phys_vehicle_ = std::unique_ptr<MultiRotor>(
        new MultiRotor(vehicle_params_.get(), vehicle_api_.get(), getKinematics(), getEnvironment()));
...
void MultirotorPawnSimApi::updateRenderedState(float dt) {
    ...
    if (getRemoteControlID() >= 0) vehicle_api_->setRCData(getRCData());
}
...
void MultirotorPawnSimApi::updateRendering(float dt) {
    ...
    // UAirBlueprintLib::LogMessage(TEXT("Collision (raw) Count:"),
